...
public class PathFinder
{
  ...
  public void DrawVisuals(bool showScoreLabels)
  {
    ...
    {
      Node2.DrawVisual(NodePos, _scores[NodePos], Color.green, showScoreLabels);
    }

    foreach (Vector3 NodePos in _closedList)
    {
      Node2.DrawVisual(NodePos, _scores[NodePos], Color.red, showScoreLabels);
    }

    foreach (Vector3 NodePos in _lastPath)
    {
      Node2.DrawVisual(NodePos, _scores[NodePos], Color.yellow, showScoreLabels);
    }
  }
  ...
  public List<Vector3> GetPaths(Vector3 startPos, Vector3 targetPosition)
  {
    ...
    Node2 startingNode = new Node2(_size, startPos, _obstacles);
    Node2 targetNode = new Node2(_size, targetPosition, _obstacles);
    ...
    while (_openList.Count > 0)
    {
      ...
      Node2 current = new Node2(_size, _openList[0], _obstacles);
      ...
      if (current != startingNode)
      {
        foreach (Vector3 nodePos in _openList)
        {
          ...
          {
            current = new Node2(_size, nodePos, _obstacles);
          }
        }
      }
      ...
      foreach (Node2 neighbourNode in current.Neighbours)
      {
